...
class Utility():
    ...
    async def calc(self, *, expr: str):
        ...
        try:
            out = ":1234: | Answer is **"
            out += str(eval(expr.replace("^", "**").replace("x", "*").replace(",", "."),
                            {'__builtins__': {}})) + "**."
        except Exception:
    ...
    async def choose(self, ctx):
        ...
        if ',' in variants and variants[-1] != ',':
            ...
            await self.bot.say("Please check your input again. " +
    ...
    async def userinfo(self, ctx, user: discord.Member = None):
        ...
        userInfo.set_footer(text=strfdelta(datetime.utcnow() - self.bot.uptime, alive_timestamp),
    ...
    async def serverinfo(self, ctx):
        ...
        serverInfo.set_footer(text=strfdelta(datetime.utcnow() - self.bot.uptime, alive_timestamp),
                              icon_url=self.bot.user.avatar_url)
    ...
    async def ping(self, ctx):
        ...
        await self.bot.edit_message(msg, "Pong! Time taken: " +
                                    str(int((msg.timestamp - ctx.message.timestamp)
                                            .microseconds // 1000)) + "ms")
