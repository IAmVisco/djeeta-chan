...
async def emolist():
	await bot.say('<https://imgur.com/a/0JqQn>\n7mb pic behind the link ')
...
async def on_message(message):
	...
	if beaver <= 0 and "beaver" in message.content.lower() and ("dead" in message.content.lower() or "ded" in message.content.lower()):
		...
		beaver = 5
	elif gm <= 0 and "GoodMorning" in message.content and message.author.bot == False:
		await bot.send_message(message.channel, "GoodMorning")
		gm = 5
	elif gn <= 0 and "GoodNight" in message.content and message.author.bot == False:
